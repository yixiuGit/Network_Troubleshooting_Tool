...
Profile the devices
    # Create a new folder
    Create Directory  ${CURDIR}/current
    ...
    Run Keyword If    ${len} == 0    Profile the system for "config;interface;platform;ospf;arp;routing;vrf;vlan" on devices "${alldevices}" as "/current/new_snapshot"
    ...    ELSE    Profile the system for "config;interface;platform;ospf;arp;routing;vrf;vlan" on devices "${alldevices}" as "./current/new_snapshot"

Compare snapshots
    Compare profile "./good/good_snapshot" with "./current/new_snapshot" on devices "${alldevices}"
